...
Más allá de la cancelabilidad (mono y epi) y la invertibilidad bilateral (iso), existen nociones intermedias que involucran la existencia de inversas unilaterales.

\begin{defi}
    Sea $f\colon A\to B$ un morfismo en una categoría $\cat C$. Decimos que $f$ es una:
    \begin{enumerate}
        \item \textbf{Sección} (o \textbf{inversa a la derecha}): si existe un morfismo $s\colon B\to A$ tal que $f \circ s = \id B$. En este caso, $f$ es necesariamente un epimorfismo (llamado \emph{epimorfismo escindido} o \emph{split epi}).
        \item \textbf{Retracción} (o \textbf{inversa a la izquierda}): si existe un morfismo $r\colon B\to A$ tal que $r \circ f = \id A$. En este caso, $f$ es necesariamente un monomorfismo (llamado \emph{monomorfismo escindido} o \emph{split mono}).
    \end{enumerate}
\end{defi}

\begin{obs}
    Es importante notar que estas condiciones son estrictamente más fuertes que ser epimorfismo o monomorfismo. 
    \begin{itemize}
        \item Si $f$ tiene una sección $s$, entonces $f$ cancela por derecha: supongamos $g f = h f$. Al componer con $s$ por la derecha obtenemos $g f s = h f s$, lo que implica $g \id B = h \id B$, es decir, $g=h$.
        \item Análogamente, si $f$ tiene una retracción $r$, entonces $f$ cancela por izquierda y es mono.
    \end{itemize}
    Sin embargo, el recíproco no es cierto en general: existen epimorfismos que no tienen sección (no todo epi se escinde) y monomorfismos que no tienen retracción. El ejemplo del orden parcial visto anteriormente ilustra esto: la flecha única $1 \to 2$ es mono y epi, pero no tiene ni sección ni retracción, pues ello implicaría la existencia de una flecha de vuelta $2 \to 1$.
\end{obs}
